...
def set_user_currency(user_id: int, currency: str):
    ...
    if row:
        # 🔄 Actualizar solo la moneda, no tocar cart ni total
...
def ensure_database():
    ...
    try:
        cursor.execute("SELECT user_id, currency, cart, total FROM users LIMIT 1")
    except sqlite3.OperationalError:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, label in CATEGORIES.items():
        keyboard.append([InlineKeyboardButton(label, callback_data=f'category:{key}')])

    keyboard.extend([
        [InlineKeyboardButton("🛒 Ver Carrito", callback_data='cart')],
        [InlineKeyboardButton("💰 Pagar", callback_data='checkout')],
        [
            InlineKeyboardButton("💱 Cambiar a USDT", callback_data='set_currency:USDT'),
            InlineKeyboardButton("💱 Cambiar a CUP", callback_data='set_currency:CUP')
        ]
    ])
...
async def show_products_by_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, price, description FROM products WHERE category = ?", (category,))
    products = cursor.fetchall()
    conn.close()

    if not products:
        await query.edit_message_text(f"No hay productos en {CATEGORIES[category]}.")
        return
    ...
    for prod in products:
        ...
        buttons.append([InlineKeyboardButton(btn_text, callback_data=f'add:{product_id}')])

    buttons.append([InlineKeyboardButton("⬅️ Volver", callback_data='start')])
    reply_markup = InlineKeyboardMarkup(buttons)
    await query.edit_message_text(f"Productos de {CATEGORIES[category]}:", reply_markup=reply_markup)
...
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute(
        "INSERT OR REPLACE INTO users (user_id, cart, total) VALUES (?, ?, ?)",
        (user_id, json.dumps(cart), 0)
    )
    conn.commit()
    conn.close()
...
async def view_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for prod_id, qty in cart.items():
        cursor.execute("SELECT name, price FROM products WHERE id = ?", (int(prod_id),))
        ...
        if prod:
            ...
            total += subtotal
            items.append(f"• {qty}x {name} → {format_currency(subtotal, currency)}")
...
async def clear_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET cart = NULL, total = 0 WHERE user_id = ?", (user_id,))
    conn.commit()
    conn.close()
...
def main():
    ...
    if not TOKEN:
        raise ValueError("❌ FALTA EL TOKEN DE TELEGRAM. Configúralo como variable de entorno.")
